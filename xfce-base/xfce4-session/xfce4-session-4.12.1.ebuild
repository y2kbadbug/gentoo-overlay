...
IUSE="debug nls policykit systemd upower +xscreensaver light-locker"
REQUIRED_USE="^^ ( xscreensaver light-locker )"
...
RDEPEND="${COMMON_DEPEND}
	...
	light-locker? ( x11-misc/light-locker )"
...
src_prepare() {
	unpack ${A}
	cd ${S}
	if use light-locker ; then
		epatch "${FILESDIR}"/${PN}-light-locker.patch
	fi
}
